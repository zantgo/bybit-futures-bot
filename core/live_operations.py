...
def get_instrument_info(symbol: str, category: str = 'linear', force_refresh: bool = False) -> Optional[Dict[str, Any]]:
    ...
    print(f"INFO [Get Instrument Info]: Consultando API para {symbol} ({category}) usando '{account_used}'...")
    ...
    try:
        ...
        if _handle_api_error_generic(response, f"Get Instrument Info for {symbol}"): return None
            ...
            if result_list:
                ...
                print(f"  ÉXITO [Get Instrument Info]: Datos obtenidos para {symbol}."); _instrument_info_cache[cache_key] = {'timestamp': now, 'data': extracted_info}; return extracted_info
            else: print(f"  INFO [Get Instrument Info]: Lista de instrumentos vacía para {symbol}."); return None
...
def set_leverage(
    ...
    print(f"INFO [Set Leverage]: Intentando establecer para {symbol} en '{target_account}': Buy={buy_lev_str}x, Sell={sell_lev_str}x")
    try:
        ...
        if not _handle_api_error_generic(response, "Set Leverage"): print(f"  ÉXITO [Set Leverage]: Apalancamiento establecido para {symbol}."); return True
        elif response and response.get('retCode') == 110043: print(f"  INFO [Set Leverage]: Apalancamiento no modificado (ya estaba) - Código 110043."); return True
        else: return False
    except InvalidRequestError as invalid_req_err:
        error_message = str(invalid_req_err)
        if "110043" in error_message or "leverage not modified" in error_message.lower(): print(f"  INFO [Set Leverage]: Apalancamiento no modificado (ya estaba) - InvalidRequestError(110043)."); return True
...
def place_market_order(
    ...
    print(f"INFO [Place Order]: Enviando orden MARKET a cuenta '{target_account}':"); print(f"  Params: {params}")
    try:
        ...
        if not _handle_api_error_generic(response, "Place Order"): order_id = response.get('result', {}).get('orderId', 'N/A'); print(f"  ÉXITO [Place Order]: Orden aceptada API. OrderID: {order_id}"); return response
...
def get_unified_account_balance_info(account_name: str) -> Optional[dict]:
    ...
    print(f"INFO [Get Unified Balance]: Obteniendo balance UNIFIED para '{account_name}'...")
    try:
        ...
        if _handle_api_error_generic(response, f"Get Unified Balance for {account_name}"): return None
            ...
            result_list = response.get('result', {}).get('list', [])
            if not result_list: print(f"  INFO [Get Unified Balance]: Sin datos para '{account_name}'."); return {'totalEquity': 0.0, 'totalAvailableBalance': 0.0, 'totalWalletBalance': 0.0, 'usdt_balance': 0.0, 'usdt_available': 0.0}
            ...
            print(f"  ÉXITO [Get Unified Balance]: Balance obtenido para '{account_name}'."); return balance_info
...
def get_funding_account_balance_info(account_name: str) -> Optional[Dict[str, Dict[str, float]]]:
    ...
    print(f"INFO [Get Funding Balance]: Obteniendo balance FUND para '{account_name}'...")
    try:
        ...
        if _handle_api_error_generic(response, f"Get Funding Balance for {account_name}"): return None
            ...
            if balance_list:
                ...
                print(f"  ÉXITO [Get Funding Balance]: Balances obtenidos para '{account_name}'. {len(funding_balances)} activo(s).")
            else: print(f"  INFO [Get Funding Balance]: Sin datos de balance para '{account_name}'.")
...
def cancel_order( symbol: str, order_id: Optional[str] = None, order_link_id: Optional[str] = None, account_name: Optional[str] = None) -> Optional[dict]:
    ...
    print(f"INFO [Cancel Order]: Intentando cancelar orden {id_type} para {symbol} en '{target_account}'...")
    try:
        ...
        if not _handle_api_error_generic(response, f"Cancel Order {id_type}"): canceled_id = response.get('result', {}).get('orderId') or response.get('result', {}).get('orderLinkId', 'N/A'); print(f"  ÉXITO [Cancel Order]: Cancelación aceptada para orden {canceled_id}."); return response
        elif response and response.get('retCode') == 110001: print(f"  INFO [Cancel Order]: Orden {id_type} no encontrada o ya finalizada (110001)."); return None
        else: return response
...
def get_order_status( symbol: str, order_id: Optional[str] = None, order_link_id: Optional[str] = None, account_name: Optional[str] = None) -> Optional[dict]:
    ...
    print(f"INFO [Get Order Status]: Buscando estado orden {id_type} en '{target_account}'...")
    try:
        ...
        if _handle_api_error_generic(response, f"Get Order Status {id_type}"): return None
            ...
            if order_list:
                ...
                if (order_id and found_id == order_id) or (order_link_id and found_link_id == order_link_id): print(f"  ÉXITO [Get Order Status]: Orden {id_type} encontrada. Estado: {order_details.get('orderStatus', 'N/A')}"); return order_details
                else: print(f"  INFO [Get Order Status]: Orden encontrada ({found_id}/{found_link_id}) no coincide con buscada ({id_type})."); return None
            else: print(f"  INFO [Get Order Status]: Orden {id_type} no encontrada."); return None
...
def get_active_position_details_api(symbol: str, account_name: Optional[str] = None) -> Optional[List[dict]]:
    ...
    print(f"INFO [Get Position]: Obteniendo detalles de posición para {symbol} en '{target_account}'...")
    try:
        ...
        if _handle_api_error_generic(response, f"Get Position for {symbol}"): return None
            ...
            if position_list:
                ...
                if active_positions: print(f"  ÉXITO [Get Position]: {len(active_positions)} posición(es) activa(s) encontrada(s) para {symbol}."); return active_positions
                else: print(f"  INFO [Get Position]: No hay posiciones activas para {symbol}."); return []
            else: print(f"  INFO [Get Position]: Lista de posiciones vacía para {symbol}."); return []
...
def close_all_symbol_positions(symbol: str, account_name: Optional[str] = None) -> bool:
    ...
    print(f"\nINFO [Close All Positions]: Intentando cerrar TODAS las posiciones para {symbol} en cuenta '{target_account}'...")
    ...
    print(f"DEBUG [Close All Positions]: Usando Precision={qty_precision}, Min Qty={min_qty}")
    ...
    if not active_positions: print(f"  INFO [Close All Positions]: No hay posiciones activas para {symbol}."); return True
    ...
    for pos in active_positions:
        ...
        try:
            ...
            if not pos_side or pos_size_float < float(min_qty): print(f"  WARN [Close All Positions]: Saltando pos inválida/pequeña (<{min_qty}): {pos}"); continue
        except (ValueError, TypeError): print(f"  WARN [Close All Positions]: Error convirtiendo tamaño {pos_size_str}. Saltando."); continue
        ...
        try:
             ...
             if qty_rounded <= Decimal(0): print(f"  WARN [Close All Positions]: Cantidad redondeada a cero para {pos_size_str}. Saltando."); all_close_attempts_made = False; continue
        ...
        print(f"  -> Intentando cerrar {pos_side} PosIdx={pos_idx} (Tamaño API: {qty_to_close_str})...")
        ...
        if not close_response or close_response.get('retCode') != 0: print(f"  -> FALLO al intentar cerrar {pos_side} PosIdx={pos_idx}."); all_close_attempts_made = False

    if all_close_attempts_made: print(f"INFO [Close All Positions]: Se intentó cerrar todas las posiciones para {symbol} en '{target_account}'.")
...
# --- NUEVA FUNCIÓN: Cerrar Posición por Lado ---
def close_position_by_side(symbol: str, side_to_close: str, account_name: Optional[str] = None) -> bool:
    ...
    print(f"\nINFO [Close Position By Side]: Buscando posición {side_to_close} para {symbol} en cuenta '{target_account}'...")
...
def get_order_execution_history(category: str, symbol: str, order_id: str, limit: int = 50) -> Optional[List[Dict[str, Any]]]:
    ...
    Args:
        category (str): Categoría ('linear', 'inverse', 'spot', 'option').
        symbol (str): Símbolo del instrumento.
        order_id (str): ID de la orden a consultar.
        limit (int): Número máximo de ejecuciones a devolver (max 100).

    Returns:
        Optional[List[Dict[str, Any]]]: Lista de diccionarios de ejecución,
